...
ggplot(Tiroide, aes(x=as.integer(Year), y=Rate,
  ...
  annotate("text", x= 1995,y= 1, size= 4,
           label="Mortandad")+ 
  scale_color_manual(values=c("New Cases"="#00674F", "Deaths_US"="pink")) + 
  guides(color="none")+
  ylab("Razón")+
  xlab("Años")+
...
ggplot(Tiroide, aes(x=as.integer(Year), y=Rate,
  ...
  ylab("Razón")+
  xlab("Años")+
  geom_point(shape="|", size=3, colour="blue", fill="yellow")+
  theme(axis.title=element_text(size=10,face="bold"))
...
cajaPR <- c(17.75, -67.4, 18.75, -65.15) # Puerto Rico bounding box coordinates the order is c(min long, min lat, max long, max lat)


bujo <- get_inat_obs(taxon_name = "Gymnasio nudipes", # Puerto Rican owl species
             quality = 'research', # only research grade observations
             geo = TRUE, # only observations with coordinates
             maxresults = 1000, # maximum number of observations
             bounds = cajaPR) # bounding box coordinates
