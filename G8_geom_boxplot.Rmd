...
Las muescas a los lados de un diagrama de caja se pueden interpretar como un intervalo de comparación alrededor de los valores de la mediana. La altura de la muesca es la mediana $+/- 1,57 x IQR/sqrt(n)$ donde IQR es el rango intercuartíl definido por los percentiles 25 y 75 y **n** es el número de puntos de datos


# Teoretical boxplot

```{r teoretical box plot, echo=TRUE}
df=as.tibble(c(1,8:10, 100:120, 125:135,150:160, 200))
df
df %>% 
  ggplot(aes(value, x=1))+ # x=1 representa que hay solamente un grupo
  geom_boxplot()


df2=as_tibble(c(2,3,2,2,4,11,8,2,5,5,3,4,2,3,3,3,4,9,2))
df2 %>% 
  ggplot(aes(value, x=1))+ # x=1 representa que hay solamente un grupo
  geom_boxplot()
...
boxplot1 <- ggplot(d1, aes(number_of_fruits, x=1))+ 
  geom_boxplot()+
           annotate("text", x= .75,y= 5, label="Fig. 1")+
...
# 2. Seleccionar variables y eliminar NA
d1 <- ElphickBirdData %>% 
  select(AQBIRDS, SITE) %>% 
  drop_na()

# 3. Box plot con notch
ggplot(d1, aes(x = factor(SITE), y = AQBIRDS)) +
  geom_boxplot(notch = TRUE) +
  labs(x = "Sitio de muestreo",
       y = "Cantidad de aves acuáticas") +
  theme(axis.text.x = element_text(angle = 90))

# 4. Guardar gráfico
ggsave("boxplot_AQBIRDS_por_SITE.png", width = 8, height = 6)
