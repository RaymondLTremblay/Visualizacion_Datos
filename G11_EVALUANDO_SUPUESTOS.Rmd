...
library(ggversa) # para los datos de la becasa de mar y la orquídea Dipodium
library(GGally) # para el gráfico de correlación
library(tidyverse) # para la manipulación de los datos
library(janitor) # para limpiar los nombres de las variables
library(reshape2) # para transformar los datos a formato largo
library(ggcorrplot) # para el gráfico de correlación con ggplot
...
La ética en estadística esta basado en comprobar y cumplir con los supuestos de las pruebas estadística que se usan.  La gran mayoría de las pruebas tradicional asume que los datos cumple con algunos supuestos, algunos típicos son. 
...
Por consecuencia es **primodial** que cada investigador cumple con evaluar los supuestos de las pruebas que usa y si no cumple que busca métodos alternos.  Por ejemplo métodos de análisis que podría usar incluye pruebas **no paramétrica** tambien **estadística bayesiana** o **métodos robusta**.  
...
# Homogeneidad de varianza
 ...
 -  2do. la variación en las tres estaciones es similar, y 
 -  3ro. la variación entre los grupos por sexo es similar. 
...
Note aquí el **-** (el signo de resta) antes de **which**, que resulta en la selección de todos los datos en la variable de "SEX" menos los "0".
...
```{r BecasaDeMarBP}
 ...
 ggplot(BecasaDeMar, aes(y=mgconsumed, fSEXO))+ 
...
dipodium <- dipodium %>%
  clean_names() %>%
  add_row(
    number_of_flowers = 60,
    height_inflo = 30
  )
dipodium
# Optional sanity check
glimpse(dipodium) # to see the structure of the data frame
```
...
```{r}
ggplot(dipodium, aes(number_of_flowers, height_inflo))+
  geom_point()+
...
En un modelo de regresión lineal clásica se asume que los errores tienen varianza constante, es decir:

$$Var(ε∣X)=σ2$$

Cuando esa varianza cambia en función de la variable explicativa, hablamos de heterocedasticidad.
En términos simples:

La dispersión de los datos alrededor de la recta de regresión no es la misma para todos los valores de X.

El caso contrario se llama homocedasticidad.
...
#df
modeldf=lm(B~A, data=df)

ggplot(df, aes(A, B))+
  geom_point()
```

## Nota que la variación en los residuales aumenta a medida que **x** aumenta

```{r Heterogeneityvariance1}
  Hetero<-ggplot(modeldf, aes(.fitted, .resid))+
  geom_point()+
  geom_hline(yintercept=0,
...
```{r Homogeneityvariance2}
  ResEst=ggplot(modelflower,
                 ...
                 geom_point(aes(colour = .stdresid >3.0),na.rm=TRUE)+
...
# hay una nueva función en el paquete ggpubr, ggqqplot que incluye los intervalo de confianza

library(ggpubr)

c= ggqqplot(Godwits, x="mgconsumed")
...
La distribución normal tiene una función especifica

$$f(x) = \frac{1}{\sqrt{2\pi\sigma^2}}e^{-\frac{(x-\mu)^2}{2\sigma^2}}$$

donde **$\mu$** es el promedio y **$\sigma$** es la desviación estándar y **$\sigma^2$** es la varianca. La **$\e$** es la base de los logaritmos naturales y **$\pi$** es la constante matemática.

En el gráfico se representa la distribución normal para los datos de la razón de consumo de la Becasa de Mar.
...
El parámetro **dnorm** quiere decir densidad de la distribución normal.  También se necesitan dos parámetros para calcular la densidad, el promedio (mean) y la desviación estándar (sd).  Con ambos hay que especificar de dónde provienen los datos; en este caso, de una variable especifica **mgconsumed**. 
...
b= ggplot(Godwits, aes(mgconsumed)) +
  ...
  stat_function(fun = dnorm, args = list(mean =
    ...
    sd = sd(Godwits$mgconsumed, na.rm = TRUE)),
    colour = "red", size = 1)+
    facet_wrap(~PERIOD) # nota que ahora la fun=dnorm es aplicado a cada grupo individualmente
...
data(SparrowsElphick)
#read_csv(
#  "/Users/raymondtremblay/Dropbox/GitHub_Dropbox_Drive/GitHub/Visualizacion_Datos/Data_files_csv/SparrowElphick.csv"
#)
...
Sparrows <- Sparrows %>%
  mutate(
    fMonth = factor(Month,
      ...
      labels = c("Mayo", "Junio", "Julio", "Agosto", "Sept.", "Oct.")
    )
  )
...
library(GGally)
ggpairs(dipodium[,c(8, 9, 13, 14, 10)], aes(colour=herbivory))
```

***

Explorando la diferencia entre las opciones de "everything", "na.or.complete", "all.obs" y "pairwise.complete.obs" con el paquete **corrplot**

```{r}
library(corrplot)
data(mtcars)
carros_mini <- mtcars[,3:6]
# remover algo datos y subsituirlos por "NA"
carros_mini[1,1] <- NA
carros_mini[2,2] <- NA

head(carros_mini) # mirar el df

library(corrplot)
corrplot(corr=cor(carros_mini, use="everything"), method="ellipse") # las variables que tienen NA no se pueden calcular
corrplot(corr=cor(carros_mini, use="na.or.complete"), method="ellipse") 
corrplot(corr=cor(carros_mini, use="pairwise.complete.obs"), method="ellipse")
#corrplot(corr=cor(carros_mini, use="all.obs"), method="ellipse") # no puede calcular nada
```
