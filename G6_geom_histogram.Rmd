...
a=ggplot(Camas_Hospital, aes(Camas, fill=factor(Year)))
a+geom_histogram(color="black", stat="bin", alpha=0.9,
                 position="dodge")+
  xlab("Número de camas por \n 1000 habitantes")+
  ylab("Frecuencia")+
  scale_y_continuous(breaks=c(0:9))+
...
```
 ...
 1. Usando el archivo de datos de **diamonds** en el paquete **ggplot2**, haga un histograma del precio de los diamantes, en diferentes **chunks**. 
  ...
  - separa el los diamantes de diferentes color en histograma diferentes
  - evalúa como se ve los gráficos con las diferentes areglos: position: “identity”, “stack”, “dodge”
  - cambia la intensidad del color
  - cambia los nombres de los ejes al español
  - salvar los gráficos en formato .png, .tiff o .jpeg y subir por lo menos uno de estos a Edmodo. 
