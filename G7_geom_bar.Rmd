...
```{r echo=FALSE}
embed_flipbook(chunk_name = "dipodiumbarrafb", display_type = c("code", "output"))
```
...
head(ASBESTOS_QUEBEC, n=12)
tail(ASBESTOS_QUEBEC)

library(janitor)

clean_names(ASBESTOS_QUEBEC)
```
...
2. Usando los datos de Anchorage, Alaska que se encuentra en la sección de datos aqui [Acchorage_Data](Data/Anchorage_ALASKA.csv), evaluar el patrón de temperatura mínima en los diferentes meses del año.  Convertir los datos de Farenheit a Celcius.  
