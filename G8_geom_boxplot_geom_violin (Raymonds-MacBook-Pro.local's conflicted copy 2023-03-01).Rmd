...
df=as.tibble(c(1, 100:250, 300:500 ,1000))
...
boxplot1 <- ggplot(d1, aes(number_of_fruits, x=1))+ 
  geom_boxplot()+
           annotate("text", x= .7,y= 6, label="Gráfico. 1")+
...
boxplot3 <- ggplot(d1, aes(number_of_fruits, x=1))+ 
  geom_boxplot(notch=TRUE,colour="blue",
           fill="orange", alpha=0.8, outlier.shape=22,
...
boxplot4 <- ggplot(d1, aes(factor(number_of_flowers), number_of_fruits))+ 
  geom_boxplot(notch=FALSE)+
           annotate("text", x= 2,y= 8, label="Fig. 4")+
...
1. Preparar un gráfico de caja (box plot) o tambien llamado **Box and whiskers**. Utiliza los datos **ElphickBirdData** en el paquete **ggversa**. Utiliza la columna **AQBIRDS** como variable continua, que son la cantidad de pájaros acuaticos muestreados por Zuur, Ieno and Elphick.  La especies de pájaro de nombre **Becasa de Mar** = **Godwit** es una ave migratoria que come pequeños moluscos en la playas. Utiliza la columna "SITE" para crear un box plot con los "notch", por cada sitio de muestreo.  
...
df2
probs <- c(0.0001, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, .99)
quantiles <- quantile(CalVal2$OH, prob=probs)
df2 <- data.frame(x=dens2$x, y=dens2$y)
df2$quant <- factor(findInterval(df2$x,quantiles))
df2

ggplot(df2, aes(x,y)) +  geom_ribbon(aes(ymin=0, ymax=y, fill=quant)) + scale_x_continuous(breaks=quantiles) + scale_fill_brewer(palette = "Set1",guide="none")+  # colores horribles
