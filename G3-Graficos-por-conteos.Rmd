...
opts_knit$set(root.dir = "~/Dropbox/GitHub_Dropbox_Drive/GitHub/Visualizacion_Datos")
```
...
library(ggversa) # la función "library" activa el paquete de **ggversa** donde se encuentra los archivos
...
```{r}
head(Anolis, n=10) # la función *head* es para visualizar las primeros 6 filas

tail(Anolis)



Anolis$DATE2=as.Date(Anolis$DATE, "%m/%d/%y") # convertir la columna de fecha de tipo *carácter* a tipo *fecha*

head(Anolis)
gt(head(Anolis)) # visualizar las primeras 6 filas de la tabla con el paquete gt
```

Las funciones para formatear la tabla son del paquete **gt** = "grammer of tables"
...
Anolis %>%  # pipe , el paquete margrite
  select(WEIGHT, SVL) %>% 
...
Anolis %>%  # pipe 
  ...
  ggplot(aes(x=WEIGHT, y=SVL, colour=SEX_AGE))+
  geom_point()+
  ylab("Snout Vent Length of *Anolis*")+
  xlab("Peso en gramos")
...
```{r echo=FALSE}
#chunk_reveal(Anolis, display_type = c("code", "output"))
```
...
Te saludo estimados <br> colegas
...
library(ggplot2)
library(ggtext)  # for markdown rendering in titles
...
Anolis %>%
  ...
  geom_point() +
  labs(
    title = "Figura: Relación de SVL y el peso de *Anolis* en <br> **El Verde**, Yunque, PR",
    y = "Snout vent length") +
  theme( plot.title = element_markdown()  # enables markdown and <br> line breaks
  )
...
ggplot(aes(SVL, WEIGHT))+
  geom_point()+
  xlab("Peso en gramos")+
  ylab("Tamaño del hocico a la cloaca")+
  theme(
    axis.title.x=                    # Cambiar el titulo del eje de x
    element_text(size=10, face="italic", family="serif",
                 colour="#EED5B7"),
    axis.title.y=                    # Cambiar el titulo del eje de x
    element_text(size=5, face="bold", 
                 colour="purple"),
...
Anolis %>%
  ...
  filter(WEIGHT<4 & SVL>20)%>%
ggplot(aes(SVL, WEIGHT))+
  geom_count(aes(size=after_stat(prop), group=SVL))+
  ylab("Peso en gramos")+
  xlab("Tamaño del hocico a la cloaca")+
  theme(axis.title=element_text(size=14,face="bold"))+
...
unique(Anolis$SEX_AGE)

Anolis %>%
  select(WEIGHT, SVL, SEX_AGE)%>%
  filter(WEIGHT<4 & SVL>20) %>%
ggplot(aes(SVL, WEIGHT, colour=SEX_AGE))+
  geom_count(aes(size=after_stat(prop), group=SEX_AGE))+
  scale_color_manual(values = c("Female" = "blue", "Juvenil" = "orange", "Male" = "red"))+
  ylab("Peso en gramos")+
  xlab("Tamaño del hocico a la cloaca")+
  theme(axis.title=element_text(size=10,face="bold"))+
...
Anolis %>%
  ...
  filter(WEIGHT<4 & SVL>20)%>%
ggplot(aes(SVL, WEIGHT, colour=SEASON))+
  geom_count(shape=24,colour="red", fill="yellow",
             aes(size=after_stat(prop), group=Survey_Site))+
  ylab("Peso en gramos")+
  xlab("Tamaño del hocico a la cloaca")+
...
Anolis %>%
  ...
  filter(WEIGHT<4 & SVL>20)%>%
ggplot(aes(SVL, WEIGHT, colour=SEASON))+
  geom_count(shape=22, stroke=1.5,alpha=.5,
             aes(size=after_stat(prop), group=Survey_Site))+
  ylab("Peso en gramos")+
  xlab("Tamaño del hocico a la cloaca")+
  facet_wrap(~Survey_Site)+
...
library(readr)
billboard <- read_csv("Billboard_data/hot-100-current.csv", locale = locale(encoding = "UTF-8"))
head(billboard)
...
get_genre_discogs <- function(song, artist, token) {
  base_url <- "https://api.discogs.com/database/search"
  query <- list(q = paste(song, artist),
                type = "release",
                token = token,
                per_page = 1)
  ...
  if (length(data$results) > 0) {
    genres <- data$results[[1]]$genre
    styles <- data$results[[1]]$style
    return(paste(c(genres, styles), collapse = ", "))
  } else {
    return(NA)
  }
}
...
#billboard <- billboard %>%
#  mutate(genre = mapply(get_genre_discogs, title, performer, MoreArgs = list(token = #token)))

#write_csv(billboard, "Billboard_data/hot-100-current-with-genres.csv")
```
