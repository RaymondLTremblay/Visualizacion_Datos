...
```{r dipodium-area2,  echo=TRUE, tidy=FALSE,  warning=TRUE, message=TRUE}
...
female_students=c(165.1, 164.6 , 162.6, 165.1,167.64, 
...
```{r}
ggplot(male_students, aes(male_students))+
  geom_density(kernel = c(kernel="gaussian"),
               alpha=0.4, fill="grey")+
  labs(y="Densidad")+
  theme(axis.title=element_text(size=10,face="bold"))
...
height <- data.frame(
  altura=c(male_students, female_students))

height
```

Rename the colums to male and female
...
Now let's produce a density graph with the height of male and female in the same figure
